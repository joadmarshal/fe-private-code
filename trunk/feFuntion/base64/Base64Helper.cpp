...
std::string Base64Helper::encode(const unsigned char *in/*转化源字串*/,int count/*转化大小*/)
{
	std::string out_str;
    unsigned char c1, c2, c3;
    int i = 0;
...
int Base64Helper::decode(const std::string in_str,unsigned char *out_str,int *strSize/*数组空间大小,若不够大,返回所需大小 */)
{
    char c1, c2, c3, c4;
    int i = 0;
    int len = in_str.length();
    ...
    while ( i<len)
    {
        ...
        // assamble the second byte
        //out_str += char( ((c2&0XF)<<4) | ((c3&0x3C)>>2) );
		out_str[k+1]=char( ((c2&0XF)<<4) | ((c3&0x3C)>>2) );
        // read the fourth byte
        do {
            c4 = in_str[i++];
            if ( c4==61 )       // meet with "=", break
